...
module Rack
  ...
  module Utils
    ...
    if defined?(Encoding) # haramako changed
    ...
    def parse_query(qs, d = nil, &unescaper)
      ...
      (qs || '').split(d ? /[#{d}] */ : DEFAULT_SEP).each do |p| # haramako changed
        next if p.empty?
    ...
    if //.respond_to?(:encoding)
      ...
      ESCAPE_HTML_PATTERN = /#{ESCAPE_HTML.keys.join("|")}/ # haramako changed
    end
