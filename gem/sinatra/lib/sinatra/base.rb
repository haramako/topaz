...
module Sinatra
  ...
  private
    ...
    class << self
      ...
      def set(option, value = (not_set = true), ignore_setter = false, &block)
        ...
        if respond_to?("#{option}=".to_sym) and not ignore_setter
    ...
    private
      ...
      def compile!(verb, path, block, options = {})
        ...
        [ pattern, keys, conditions, proc { |a,p| unbound_method.bind(a).call(*p) } ] # haramako changed
      end
    ...
    private
      def setup_default_middleware(builder)
        # builder.use ExtendedRack # haramako deleted
      ...
      def inherited(subclass)
        subclass.reset!
        # subclass.set :app_file, caller_files.first unless subclass.app_file? # haramako changed
        super
      end
    ...
    set :server, %w[webrick thin mongrel webrick]
    set :bind, '0.0.0.0'
    set :port, 4567
  ...
  class Application < Base
    ...
    def self.register(*extensions, &block) #:nodoc:
      ...
      DelegatorX.delegate(*added_methods)
  ...
  # at the top-level.
  module DelegatorX #:nodoc: # haramako changed
    def self.delegate(*methods)
      methods.each do |method_name|
        define_method(method_name) do |*args, &block|
          # return super(*args, &block) if respond_to? method_name # haramako deleted
          DelegatorX.target.send(method_name, *args, &block)
        end
        private method_name
      end
    end
  ...
  def self.register(*extensions, &block)
    DelegatorX.target.register(*extensions, &block)
  end

  # Include the helper modules provided in Sinatra's request context.
  def self.helpers(*extensions, &block)
    DelegatorX.target.helpers(*extensions, &block)
  end

  # Use the middleware for classic applications.
  def self.use(*args, &block)
    DelegatorX.target.use(*args, &block)
  end
end
